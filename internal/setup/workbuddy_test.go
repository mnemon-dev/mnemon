...
func TestWorkBuddyWriteSkill(t *testing.T) {
	dir := t.TempDir()

	skillPath, err := WorkBuddyWriteSkill(dir)
	if err != nil {
	...
	if !strings.Contains(string(data), "WorkBuddy") {
		t.Fatalf("workbuddy skill should mention WorkBuddy: %s", string(data))
	}
}

func TestWorkBuddyWriteHook(t *testing.T) {
	dir := t.TempDir()

	hookPath, err := WorkBuddyWriteHook(dir, "prime.sh", []byte("#!/bin/bash\n"))
	if err != nil {
...
func TestWorkBuddyRegisterHooksPreservesUnrelatedConfig(t *testing.T) {
	dir := t.TempDir()
  ...
  "other": true
	...
	if _, err := WorkBuddyRegisterHooks(dir); err != nil {
	...
	if _, ok := stop[0].(map[string]any)["loop_limit"]; ok {
		t.Fatalf("workbuddy hook schema should not include loop_limit: %#v", stop[0])
	}
}

func TestWorkBuddyEjectRemovesOnlyMnemonFilesAndHooks(t *testing.T) {
	dir := t.TempDir()
	if _, err := WorkBuddyWriteSkill(dir); err != nil {
		t.Fatalf("write skill: %v", err)
	}
	if _, err := WorkBuddyWriteHook(dir, "prime.sh", []byte("#!/bin/bash\n")); err != nil {
		t.Fatalf("write hook: %v", err)
	}
	if _, err := WorkBuddyRegisterHooks(dir); err != nil {
		t.Fatalf("register hooks: %v", err)
